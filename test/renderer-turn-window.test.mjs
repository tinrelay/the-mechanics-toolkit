...
assert.equal(count(app, projectionSource), buildProfiled ? 0 : 1,
  "final projection helper matches selector generation");
assert.ok(buildProfiled ? app.includes(",UHrendererTail,") : app.includes(`,${tailSource},${projectionSource},`),
...
if (!buildProfiled) assert.match(app, /var [$A-Z_a-z][$\w]*(?:,[$A-Z_a-z][$\w]*)*,UHrendererTail,UHrendererProjection,[$A-Z_a-z][$\w]*(?:,[$A-Z_a-z][$\w]*)*=[$A-Z_a-z][$\w]*\(\(\(\)=>\{/,
