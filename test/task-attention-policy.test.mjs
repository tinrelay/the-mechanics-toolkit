...
testRosterAttention(app, primary);
...
function testRosterAttention(appSource, appPrimarySource) {
  ...
  const boundary = appSource.indexOf(`function PYs(){MTKuseAttentionBootstrap${suffix}();`, start);
  ...
  for (const contract of [
    `MTKattentionPolicyAtom=${linux ? "Fp" : "Ip"}(Q,0)`,
  ...
  const rowContracts = linux ? linuxBuild9647.applied.primary : [
